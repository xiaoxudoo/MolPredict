...
exports.run_example = function (req, res, next) {
  ...
  exec(cmdStr, function (err, stdout, stderr) {
    if (stdout && stdout > 14) {
      ...
      py = spawn('python', [`${python_path}/mol3dopt_py/ca_all_${routeName}.py`]);
      const data = [];
      let dataString = '';
...
exports.cal_opt_step_one = function (req, res, next) {
  ...
  const calTypePy = req.body.runType ? `${python_path}/mol3dopt_py/ca_${req.body.runType}_${routeName}.py` : `${python_path}/mol3dopt_py/ca_${routeName}.py`
  ...
  exec(cmdStr, function (err, stdout, stderr) {
    ...
    if (!req.session.user) {
      if (isRunning) { // python计算的加锁条件
        ...
        const { spawn } = require('child_process');
        py = spawn('python', [`${python_path}/mol3dopt_py/ca_replace_mol3dopt.py`]);
        const data = [];
        let dataString = '';
...
exports.query_opt_result = function (req, res, next) {
  ...
  Order.get(orderId, function (err, ret) {
    ...
    if (results == '' || results == null) {
      res.render(`drug/pc/mol3dopt/error.jade`, { 'error': 'The input is incorrect. Please have a check.', 'accesscount': pvcount(0) })
    }
    //  deal json string
    var json = JSON.parse(results.replace(/\\/g, '').replace(/\"\[/g, '[').replace(/\]\"/g, ']'));
    res.render(`drug/pc/mol3dopt/optimisation.jade`, { 'items': json, 'accesscount': pvcount(0) });
  })
}
