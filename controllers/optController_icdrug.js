  ...
  exec(cmdStr, function (err, stdout, stderr) {
    ...
    if (!req.session.user) {
      if (isRunning) { // python计算的加锁条件
        ...
        const { spawn } = require('child_process');
        py = spawn('python', [`${python_path}/molopt_py/ca_replace_molopt_icdrug.py`]);
        const data = [];
        let dataString = '';
...
exports.query_opt_result = function (req, res, next) {
  ...
  Order.get(orderId, function (err, ret) {
    ...
    if (results == '' || results == null) {
      res.render(`drug/pc/molopt_icdrug/error.jade`, { 'error': 'The input is incorrect. Please have a check.', 'accesscount': pvcount(0) })
    }
    //  deal json string
    var json = JSON.parse(results.replace(/\\/g, '').replace(/\"\[/g, '[').replace(/\]\"/g, ']'));
    res.render(`drug/pc/molopt_icdrug/optimisation.jade`, { 'items': json, 'accesscount': pvcount(0) });
  })
}
