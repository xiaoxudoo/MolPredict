  ...
  exec(cmdStr, function (err, stdout, stderr) {
    if (stdout && stdout > 14) {
      ...
      var cp = require('child_process'),
        py = cp.spawn('python', [`${python_path}/molgpka_py/ca_hep_${routeName}.py`]),
...
exports.cal_pka = function (req, res, next) {
  const routeName = 'molgpka';
  ...
  exec(cmdStr, function (err, stdout, stderr) {
    ...
    if (!req.session.user) {
      if (isRunning) { // python计算的加锁条件
        ...
        const calTypePy = req.body.runType ? `${python_path}/molgpka_py/ca_${req.body.runType}_${routeName}.py` : `${python_path}/molgpka_py/ca_${routeName}.py`
...
exports.query_pka_result = function (req, res, next) {
  ...
  Order.get(orderId, function (err, ret) {
    ...
    if (results == '' || results == null) {
      res.render(`drug/pc/molgpka/error.jade`, { 'error': 'The input is incorrect. Please have a check.', 'accesscount': pvcount(0) })
    }
    //  deal json string
    var json = JSON.parse(results.replace(/\\/g, '').replace(/\"\[/g, '[').replace(/\]\"/g, ']'));
    res.render(`drug/pc/molgpka/absorption.jade`, { 'items': json, 'accesscount': pvcount(0) })
  })
}
